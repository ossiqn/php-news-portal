...
if ($isEdit && $pdo) {
    ...
    $stmt->execute([$newsId]);
    $n = $stmt->fetch() ?: [];
    if (!$n) { header('Location: ' . ADMIN_BASE . '/haberler/'); exit; }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $pdo) {
    ...
    if (!$title) {
        ...
        try {
            if ($isEdit) {
                $updateCols = "title=?,slug=?,summary=?,content=?,category_id=?,image=?,status=?,is_featured=?,is_breaking=?,published_at=?,updated_at=NOW(),views=IF(?=0,views,?)";
                $updateParams = [$title,$slug,$summary,$content,$catId?:null,$image,$status,$is_featured,$is_breaking,$pubDate,$views_set,$views_set,$newsId];
                // author, tags alanları varsa ekle
                foreach([['author'=>$author],['tags'=>$keywords]] as $extra) {
                    foreach($extra as $col=>$val) {
                        try {
                            $pdo->prepare("SELECT $col FROM news LIMIT 1")->execute();
                            $updateCols .= ",$col=?";
                            array_splice($updateParams,-1,0,[$val]);
                        } catch(PDOException $e2){}
                    }
                }
                $pdo->prepare("UPDATE news SET $updateCols WHERE id=?")->execute($updateParams);
                ...
                $insCols = "title,slug,summary,content,category_id,image,status,is_featured,is_breaking,views,published_at";
                $insParams = [$title,$slug,$summary,$content,$catId?:null,$image,$status,$is_featured,$is_breaking,$views_set,$pubDate];
                $insPlc = str_repeat('?,',11).'?';
                foreach([['author'=>$author],['tags'=>$keywords]] as $extra) {
                    foreach($extra as $col=>$val) {
                        try {
                            $pdo->prepare("SELECT $col FROM news LIMIT 1")->execute();
                            $insCols .= ",$col"; $insParams[] = $val; $insPlc .= ',?';
                        } catch(PDOException $e2){}
                    }
                }
                $pdo->prepare("INSERT INTO news ($insCols) VALUES ($insPlc)")->execute($insParams);
                $newId = (int)$pdo->lastInsertId();
                logAction('Haber Eklendi.', $title);
                header('Location: ' . ADMIN_BASE . '/haberler/ekle.php?id='.$newId.'&saved=1');
                exit;
            }
...
$breadcrumbs = [['label'=>'Haberler','url'=>ADMIN_BASE.'/haberler/'],['label'=>$pageTitle]];

require __DIR__ . '/../includes/layout.php';
?>

<style>
.news-form{display:grid;grid-template-columns:1fr 290px;gap:20px;align-items:start}
.news-main{min-width:0}
.news-side{position:sticky;top:60px}
...
@media(max-width:1000px){.news-form{grid-template-columns:1fr}.news-side{position:static}}
</style>
...
<div style="display:flex;align-items:center;justify-content:space-between;background:var(--c1);padding:10px 16px;border-radius:var(--r);margin-bottom:18px;">
    ...
    <div style="display:flex;gap:8px;">
        <a href="<?= ADMIN_BASE ?>/haberler/" class="btn btn-secondary">
...
<div class="news-main">
    ...
    <div style="display:flex;gap:10px;justify-content:flex-end;padding:6px 0;">
        <a href="<?= ADMIN_BASE ?>/haberler/" class="btn btn-secondary btn-lg">İptal</a>
        <?php if ($isEdit): ?><a href="<?= SITE_BASE ?>/haber/<?= sTr($n['slug']??'') ?>" target="_blank" class="btn btn-purple btn-lg">Haberi Gör</a><?php endif; ?>
