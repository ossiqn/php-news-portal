...
if(!$news){ header('HTTP/1.0 404 Not Found'); header('Location: /'); exit; }
...
@media(min-width:1200px){.news-layout{grid-template-columns:1fr 320px!important}.news-title{font-size:30px}}
@media(max-width:900px){.news-layout{grid-template-columns:1fr!important}.news-body{padding:18px}.news-title{font-size:22px}.related-grid{grid-template-columns:1fr 1fr}}
@media(max-width:600px){.news-body{padding:14px}.news-title{font-size:19px}.related-grid{grid-template-columns:1fr}.news-share{flex-wrap:wrap}}
</style>
...
<article class="news-article">
    ...
    <div class="news-hero"><img src="<?= h(imgUrl($news['image'])) ?>" alt="<?= h($news['title']) ?>"></div>
    <?php endif; ?>
    <div class="news-body">
        <div class="news-breadcrumb">
            <a href="/">Ana Sayfa</a><span>›</span>
            <?php if(!empty($news['cat_name'])): ?>
            <a href="/<?= h($news['cat_slug']??'') ?>"><?= h($news['cat_name']) ?></a><span>›</span>
            <?php endif; ?>
        ...
        <div class="news-tags">
            ...
            <a href="<?= $_base ?>/arama?q=<?= urlencode($tag) ?>" class="news-tag"># <?= h($tag) ?></a>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>
    ...
    <div class="news-body" style="border-top:1px solid var(--border);padding-top:24px">
        ...
        <div class="related-grid">
            ...
            <div class="rc">
                <div class="rc-img"><img src="<?= h(imgUrl($r['image'])??'https://placehold.co/400x225/eee/999?text=H') ?>" alt="<?= h($r['title']) ?>"></div>
                <div class="rc-body">
...
<aside style="position:sticky;top:60px;display:flex;flex-direction:column;gap:20px">
    <div style="background:var(--bg-card);border-radius:var(--r);box-shadow:var(--sh);overflow:hidden">
        ...
        <div style="padding:6px 0">
            ...
            <a href="<?= $_base ?>/haber/<?= h($mr['slug']) ?>" style="display:flex;align-items:flex-start;gap:10px;padding:10px 15px;border-bottom:1px solid var(--border);transition:background .15s" onmouseover="this.style.background='var(--bg)'" onmouseout="this.style.background=''">
