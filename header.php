...
<link rel="alternate" type="application/rss+xml" title="<?= h($_siteName) ?> RSS" href="<?= $_base ?>/rss">
...
@media(min-width:1200px){
    .top-header-inner{padding:14px 0}
    .brand-text .brand-name{font-size:24px}
    .search-box input{width:220px}
    .nav-link{padding:0 16px;font-size:12px}
}
...
@media(max-width:680px){
    ...
    .top-header-inner{padding:10px 0}
}
    ...
    <div class="container" style="display:flex;align-items:center;height:100%;padding:0 18px;max-width:100%">
        ...
        <div class="breaking-track-wrap">
            <div class="breaking-track">
                ...
                <span class="breaking-item"><a href="<?= $_base ?>/haber/<?= h($b['slug']) ?>" style="color:inherit"><?= h($b['title']) ?></a></span>
                <?php endforeach; ?>
            </div>
        </div>
    </div>
...
<header class="top-header">
    <div class="container">
        <div class="top-header-inner">
            <a href="<?= $_base ?: '/' ?>" class="brand">
                    <?php if($_logoPath): ?>
                <img src="<?= h($_logoPath) ?>" alt="<?= h($_siteName) ?>" class="brand-logo-img">
                <?php else: ?>
            ...
            <div class="header-right">
                ...
                <form class="search-box" action="<?= $_base ?>/arama" method="get">
...
<nav class="main-nav">
    <div class="container">
        <div class="nav-inner">
            <a href="<?= $_base ?: '/' ?>" class="nav-link <?= $_currentPage==='index'?'active':'' ?>">Ana Sayfa</a>
            <div class="nav-item">
                <span class="nav-link">Haberler <span class="arr">▼</span></span>
                <div class="nav-mega">
                    <?php foreach($_allCats as $nc): ?>
                    <a href="/<?= h($nc['slug']) ?>"><?= h($nc['name']) ?></a>
                    <?php endforeach; ?>
                </div>
            </div>
            <a href="<?= $_base ?>/gundem" class="nav-link">Gundem</a>
            <a href="<?= $_base ?>/siyaset" class="nav-link">Siyaset</a>
            <a href="<?= $_base ?>/ekonomi" class="nav-link">Ekonomi</a>
            <a href="<?= $_base ?>/spor" class="nav-link">Spor</a>
            <a href="<?= $_base ?>/dunya" class="nav-link">Dunya</a>
            <a href="<?= $_base ?>/teknoloji" class="nav-link">Teknoloji</a>
            <div class="nav-item">
                <span class="nav-link">Hizmetler <span class="arr">▼</span></span>
                <div class="nav-dd">
                    <a href="<?= $_base ?>/hava-durumu">Hava Durumu</a>
                    <a href="<?= $_base ?>/namaz-vakitleri">Namaz Vakitleri</a>
                    <a href="<?= $_base ?>/nobetci-eczaneler">Nobetci Eczaneler</a>
                    <a href="<?= $_base ?>/gazete-arsivi">Gazete Arsivi</a>
                </div>
            </div>
            <div class="nav-item">
                <span class="nav-link">Daha Fazla <span class="arr">▼</span></span>
                <div class="nav-dd">
                    <a href="<?= $_base ?>/gunun-haberleri">Gunun Haberleri</a>
                    <a href="<?= $_base ?>/arsiv">Arsiv</a>
                    <a href="<?= $_base ?>/arama">Arama</a>
                    <a href="<?= $_base ?>/uye-paneli">Uye Paneli</a>
                    <a href="<?= $_base ?>/rss" style="color:#f97316!important;font-weight:700!important">
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="#f97316" style="margin-right:3px"><path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/></svg>
                        RSS Beslemeleri
                    </a>
                </div>
            </div>
        ...
        <div class="nav-hamburger">
            <a href="<?= $_base ?: '/' ?>" style="color:#fff;font-family:var(--fh);font-weight:900;font-size:16px;flex-shrink:0"><?= h($_siteName) ?></a>
            <form class="mob-search" action="<?= $_base ?>/arama" method="get">
    ...
    <div class="mob-menu" id="mobMenu">
        <a href="<?= $_base ?: '/' ?>"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>Ana Sayfa</a>
        <div class="mob-cat">Haberler</div>
        <?php foreach($_allCats as $mc): ?>
        <a href="/<?= h($mc['slug']) ?>" style="padding-left:28px"><?= h($mc['name']) ?></a>
        <?php endforeach; ?>
