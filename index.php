...
@media(max-width:600px){
    ...
    .g2{grid-template-columns:1fr!important}
    .pg-wrap{padding:14px 0 40px}
    .ic{height:220px!important}
    .today-temp{font-size:56px!important}
}
</style>
...
<div style="display:grid;grid-template-columns:1.55fr 1fr;gap:var(--gap);margin-bottom:28px" class="g2">
    <a href="<?= $_base ?>/haber/<?= h($heroMain['slug']) ?>" class="ic" style="height:420px">
        <img src="<?= h(imgUrl($heroMain['image']) ?: 'https://placehold.co/400x225/eee/999?text=H') ?>" alt="<?= h($heroMain['title']) ?>">
    ...
    <div style="display:flex;flex-direction:column;gap:var(--gap)">
        ...
        <a href="<?= $_base ?>/haber/<?= h($hs['slug']) ?>" class="ic" style="flex:1;min-height:196px">
            <img src="<?= h(imgUrl($hs['image']) ?: 'https://placehold.co/400x225/eee/999?text=H') ?>" alt="<?= h($hs['title']) ?>">
            <div class="ic-grad"></div>
            <div class="ic-body">
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:var(--gap);margin-bottom:36px" class="g3">
    ...
    <a href="<?= $_base ?>/haber/<?= h($hs['slug']) ?>" class="ic" style="height:180px">
        <img src="<?= h(imgUrl($hs['image']) ?: 'https://placehold.co/400x225/eee/999?text=H') ?>" alt="<?= h($hs['title']) ?>">
        <div class="ic-grad"></div>
        <div class="ic-body">
            <?= badge($hs['cat_name']??'Haber',$hs['cat_color']??'#cc1a25','xs') ?>
...
<div style="margin-bottom:44px">
    <div class="sec-h" style="--ac:<?= h($ac) ?>">
        <h2><?= h($cat['name']) ?></h2>
        <a href="/<?= h($cat['slug']) ?>" class="all">Tumu &rsaquo;</a>
    </div>
    <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:var(--gap)" class="g2">
        <a href="<?= $_base ?>/haber/<?= h($main['slug']) ?>" class="ic" style="height:260px">
            <img src="<?= h(imgUrl($main['image']) ?: 'https://placehold.co/600x380/0c1a2e/fff?text='.urlencode($cat['name'])) ?>" alt="<?= h($main['title']) ?>">
            <div class="ic-grad"></div>
            <div class="ic-body">
        ...
        <div>
            ...
            <div class="li">
                <div class="li-thumb"><img src="<?= h(imgUrl($r['image']) ?: 'https://placehold.co/400x225/eee/999?text=H') ?>" alt="<?= h($r['title']) ?>"></div>
                <div class="li-body">
                    <h4><a href="<?= $_base ?>/haber/<?= h($r['slug']) ?>"><?= h(mb_substr($r['title'],0,65)) ?></a></h4>
...
<aside style="position:sticky;top:60px;display:flex;flex-direction:column;gap:22px">
    ...
    <div style="background:var(--bg-card);border-radius:var(--r);box-shadow:var(--sh);overflow:hidden">
        ...
        <div style="padding:8px 0">
            ...
            <a href="<?= $_base ?>/haber/<?= h($mr['slug']) ?>" style="display:flex;align-items:flex-start;gap:10px;padding:10px 16px;border-bottom:1px solid var(--border);transition:background .15s" onmouseover="this.style.background='var(--bg)'" onmouseout="this.style.background=''">
    ...
    <div style="background:var(--navy);border-radius:var(--r);overflow:hidden">
        ...
        <a href="<?= $_base ?>/haber/<?= h($tr['slug']) ?>" style="display:flex;gap:10px;align-items:flex-start;padding:11px 16px;border-bottom:1px solid rgba(255,255,255,.06);transition:background .15s" onmouseover="this.style.background='rgba(255,255,255,.04)'" onmouseout="this.style.background=''">
            <div style="width:72px;height:50px;border-radius:var(--r);overflow:hidden;flex-shrink:0"><img src="<?= h(imgUrl($tr['image']) ?: 'https://placehold.co/400x225/eee/999?text=H') ?>" alt="<?= h($tr['title']) ?>"></div>
            <div>
    ...
    <div style="background:var(--bg-card);border-radius:var(--r);box-shadow:var(--sh);overflow:hidden">
        ...
        <div style="padding:10px 16px 14px;display:flex;flex-wrap:wrap;gap:6px">
            ...
            <a href="/<?= h($ac2['slug']) ?>" style="display:inline-block;background:var(--bg);border:1px solid var(--border);border-radius:2px;padding:4px 10px;font-size:11px;font-weight:600;color:var(--text-2);transition:all .15s" onmouseover="this.style.background='<?= h($ac2['color']??'#cc1a25') ?>';this.style.color='#fff';this.style.borderColor='transparent'" onmouseout="this.style.background='var(--bg)';this.style.color='var(--text-2)';this.style.borderColor='var(--border)'"><?= h($ac2['name']) ?></a>
            <?php endforeach; ?>
        </div>
    </div>
